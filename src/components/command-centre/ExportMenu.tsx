  ...
  const run = (format: ExportFormat) => {
    ...
    try {
      ...
      toast.success("Export started", {
        description: `${format.toUpperCase()} generated from "${active.name}" and the current filters.`,
      });
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose which sections appear in the PDF, Excel, CSV and slide deck outputs.
          </DialogDescription>
        </DialogHeader>
